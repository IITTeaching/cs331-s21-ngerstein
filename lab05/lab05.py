...
class LinkedList:
    ...
    def __delitem__(self, idx):
        ...
        for i in range(idx):
            banana = banana.next
        banana.prior.next = banana.next
        banana.next.prior = banana.prior
        self.length += -1
        ### END SOLUTION
    ...
    def cursor_insert(self, value):
        ...
        ### BEGIN SOLUTION
        banana = self.cursor
        n = LinkedList.Node(value, prior= banana, next=banana.next)
        banana.next.prior = banana.next = n
        self.length += 1
        self.cursor = n
